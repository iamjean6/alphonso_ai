...
const SideBar = ({ userData, isOpen, onClose, onLogout, onNewChat, onSessionSelect, activeSessionId }) => {
        ...
        <>
            ...
            `}>
                ...
                <nav className="space-y-1 mb-2">
                    ...
                    <NavItem icon={<Grid size={20} />} label="Sports" />
                    ...
                    <div className="space-y-1">
                        {sessions.map((session) => (
                            <div
                                ...
                                className={`px-3 py-3 rounded-xl text-sm transition-all cursor-pointer truncate ${activeSessionId === session.sessionId ? 'bg-sidebar-accent text-[var(--accent-sport)] border-l-2 border-[var(--accent-sport)]' : 'text-sidebar-foreground/60 hover:bg-sidebar-accent hover:text-sidebar-foreground'
                                    }`}
                            >
                                {session.title || "New Performance Chat"}
                            </div>
                        ))}
                    </div>
                </div>
